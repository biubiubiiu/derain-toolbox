# This code is taken from https://github.com/perrying/guided-filter-pytorch

import torch
import torch.nn as nn
import torch.nn.functional as F

from .box_filter import boxfilter2d
...
def guidedfilter2d_color(guide, src, radius, eps, scale=None):
    ...
    var_I_rr = boxfilter2d(guide_r * guide_r, radius) / \
        N - mean_I_r * mean_I_r + eps  # b x 1 x H x W
    var_I_rg = boxfilter2d(guide_r * guide_g, radius) / \
        N - mean_I_r * mean_I_g  # b x 1 x H x W
    var_I_rb = boxfilter2d(guide_r * guide_b, radius) / \
        N - mean_I_r * mean_I_b  # b x 1 x H x W
    var_I_gg = boxfilter2d(guide_g * guide_g, radius) / \
        N - mean_I_g * mean_I_g + eps  # b x 1 x H x W
    var_I_gb = boxfilter2d(guide_g * guide_b, radius) / \
        N - mean_I_g * mean_I_b  # b x 1 x H x W
    var_I_bb = boxfilter2d(guide_b * guide_b, radius) / \
        N - mean_I_b * mean_I_b + eps  # b x 1 x H x W
    ...
    # inverse
    inv_var_I_rr = (var_I_gg * var_I_bb - var_I_gb *
                    var_I_gb) / cov_det  # b x 1 x H x W
    inv_var_I_rg = - (var_I_rg * var_I_bb - var_I_rb *
                      var_I_gb) / cov_det  # b x 1 x H x W
    inv_var_I_rb = (var_I_rg * var_I_gb - var_I_rb *
                    var_I_gg) / cov_det  # b x 1 x H x W
    inv_var_I_gg = (var_I_rr * var_I_bb - var_I_rb *
                    var_I_rb) / cov_det  # b x 1 x H x W
    inv_var_I_gb = - (var_I_rr * var_I_gb - var_I_rb *
                      var_I_rg) / cov_det  # b x 1 x H x W
    inv_var_I_bb = (var_I_rr * var_I_gg - var_I_rg *
                    var_I_rg) / cov_det  # b x 1 x H x W
    ...
    cov_Ip = torch.stack([cov_Ip_r, cov_Ip_g, cov_Ip_b],
                         1)  # b x 3 x C x H x W

    a = torch.einsum("bichw,bijhw->bjchw", (cov_Ip, inv_sigma))
    b = mean_p - a[:, 0] * mean_I_r - a[:, 1] * \
        mean_I_g - a[:, 2] * mean_I_b  # b x C x H x W

    mean_a = torch.stack(
        [boxfilter2d(a[:, i], radius) / N for i in range(3)], 1)
    mean_b = boxfilter2d(b, radius) / N

    if scale is not None:
        guide = guide_sub
        mean_a = torch.stack([F.interpolate(
            mean_a[:, i], guide.shape[-2:], mode='bilinear') for i in range(3)], 1)
